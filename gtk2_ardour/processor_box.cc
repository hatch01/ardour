	...
	, _plugin_display(0)
{
	...
	if (_processor) {
		...
		/* Allow Ctrl+Middle-Click MIDI learn on the processor LED for
		 * plugin inserts. bypass_control() always returns a valid
		 * controllable (either the plugin's own bypass port or the
		 * host-level automation control created for plugins without one). */
		if (pi) {
